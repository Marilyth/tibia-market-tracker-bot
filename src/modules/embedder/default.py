...
def get_default_embed():
    ...
    embed.set_footer(text="Tibia Market API")
...
def get_default_error_embed(command_name: str, exception: discord.ext.commands.CommandError):
    ...
    exception_expression = str(exception).rsplit("raised an exception: ", maxsplit=1)[-1]
    embed.add_field(name="Error", value=f"```{exception_expression}```")
